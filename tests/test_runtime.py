...
class RuntimeTests(unittest.TestCase):
    ...
    def test_heartbeat_renders_corpse_after_death_action(self) -> None:
        now = datetime(2026, 1, 1, tzinfo=timezone.utc)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            state.current_folder = str(desktop)
            state.hunger = 70
            (desktop / "slipperyfloor.hazard").write_text("", encoding="utf-8")
            save_state(paths.state_file, state)

            frame = run_heartbeat(paths, now)

            self.assertEqual(frame.action.action_id, "die")
            self.assertTrue((desktop / "dead_boogart.png").exists())
